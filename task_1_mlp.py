...
class MLP(nn.Module):
    def __init__(self):
        ...
        # Input dimensions: 128x128 image with 3 color channels + 4 action classes
        self.input_dim = (128 * 128 * 3) + 4 
    ...
    def forward(self, img, action):
        x = self.flatten(img)
        # Convert action index to one-hot encoding and concatenate
...
def train(model, loader, device, epochs=20):
    ...
    print("Loss plot saved to loss_mlp.png.")
...
def test(model, loader, device):
    ...
    print(f"\n>>> MLP Test Error (MSE): {avg_test_error:.6f}")
