...
def collect(idx, N):
    ...
    imgs_next = torch.zeros(N, 3, 128, 128, dtype=torch.uint8)
        
    for i in range(N):
        print(f"Processo {idx}: Campione {i+1}/{N}")
        env.reset() 

        # capturing the initial state for training the network
        _, img_before = env.state()
        
        action_id = np.random.randint(4)
        env.step(action_id)
        
       # capturing the final state after the action
        pos_after, img_after = env.state()
        
        positions[i] = torch.tensor(pos_after)
        actions[i] = action_id
        imgs[i] = img_before
        imgs_next[i] = img_after

    torch.save(positions, f"positions_{idx}.pt")
    torch.save(actions, f"actions_{idx}.pt")
    torch.save(imgs, f"imgs_{idx}.pt")
    torch.save(imgs_next, f"imgs_next_{idx}.pt")
    print(f"Processo {idx}: Completato e salvato.")


if __name__ == "__main__":
    processes = []
    # 4 processi x 250 campioni = 1000 dati totali
    samples_per_process = 250
    
    for i in range(4):
        print(f"Avviando processo {i} per raccogliere {samples_per_process} campioni...")
        p = Process(target=collect, args=(i, samples_per_process))
        p.start()
        processes.append(p)
